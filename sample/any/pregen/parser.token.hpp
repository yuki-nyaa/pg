...
struct Token_Settings{
    ...
    const char* token_name[token_total] = {"List","Pair","(",")","EOF_",};
    ...
    using Token_Tp = yuki::pg::Token<Token_Kind_t,kind_p,Ts...>;

    struct Token{
        using List = Token_Tp<Token_Kind::List,size_t,std::list<size_t>>;
        using Pair = Token_Tp<Token_Kind::Pair,size_t,std::list<size_t>>;
        using LB = Token_Tp<Token_Kind::LB>;
        using RB = Token_Tp<Token_Kind::RB>;
        using EOF_ = Token_Tp<Token_Kind::EOF_>;
    };
